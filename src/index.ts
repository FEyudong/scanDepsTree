...
import { isNpmPath, getConfig } from "./utils";
 ...
 * @returns DepTree
 */
function createDepsTree(path: string): DepTree {
  // 复用已解析过的依赖节点
 ...
 * @returns 依赖数组
 */
export function getDeps(cwf: string) {
  // console.log("cwf", cwf);
  ...
  try {
    ...
    traverse.default(ast, {
      enter(path) {
        ...
        const resolvedPath = transformImportPath(
          importPath,
          pathTools.dirname(cwf)
        );
 ...
 * @returns 绝对文件路径
 */
function transformImportPath(importPath: string, cwd: string) {
  if (isNpmPath(importPath)) {
    return;
  }
  ...
  if (!pathTools.extname(_path)) {
    for (let i = 0; i < DEFAULT_EXTENSIONS.length; i++) {
      ...
      // eg: ./utils
      const addExtAndIndexPath = pathTools.resolve(
        cwd,
        `${_path}/index${extName}`
      );
